...
class gpu_hash_table {
 ...
 public:
  ...
  gpu_hash_table(uint32_t max_keys, uint32_t num_buckets,
                 const uint32_t device_idx, const int64_t seed,
                 const bool req_values = true, const bool identity_hash = false,
    ...
    CUDA_CALL(cudaSetDevice(device_idx_));

    // allocating key, value arrays:
    CUDA_CALL(cudaMalloc((void**)&d_key_, sizeof(KeyT) * max_keys_));
    if (req_values_) {
      CUDA_CALL(cudaMalloc((void**)&d_value_, sizeof(ValueT) * max_keys_));
    }
    CUDA_CALL(cudaMalloc((void**)&d_query_, sizeof(KeyT) * max_keys_));
    CUDA_CALL(cudaMalloc((void**)&d_result_, sizeof(ValueT) * max_keys_));
    CUDA_CALL(cudaMalloc((void**)&d_count_, sizeof(uint32_t) * max_keys_));
  ...
  ~gpu_hash_table() {
    CUDA_CALL(cudaSetDevice(device_idx_));
    CUDA_CALL(cudaFree(d_key_));
    if (req_values_) {
      CUDA_CALL(cudaFree(d_value_));
    }
    CUDA_CALL(cudaFree(d_query_));
    CUDA_CALL(cudaFree(d_result_));
    CUDA_CALL(cudaFree(d_count_));
  ...
  float hash_build(KeyT* h_key, ValueT* h_value, uint32_t num_keys) {
    ...
    CUDA_CALL(cudaSetDevice(device_idx_));
    CUDA_CALL(cudaMemcpy(d_key_, h_key, sizeof(KeyT) * num_keys,
                         cudaMemcpyHostToDevice));
    if (req_values_) {
      CUDA_CALL(cudaMemcpy(d_value_, h_value, sizeof(ValueT) * num_keys,
                           cudaMemcpyHostToDevice));
    }
  ...
  float hash_build_with_unique_keys(KeyT* h_key, ValueT* h_value,
                                    uint32_t num_keys) {
    // moving key-values to the device:
    CUDA_CALL(cudaSetDevice(device_idx_));
    CUDA_CALL(cudaMemcpy(d_key_, h_key, sizeof(KeyT) * num_keys,
  ...
  float hash_search(KeyT* h_query, ValueT* h_result, uint32_t num_queries) {
    CUDA_CALL(cudaSetDevice(device_idx_));
    CUDA_CALL(cudaMemcpy(d_query_, h_query, sizeof(KeyT) * num_queries,
                         cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMemset(d_result_, 0xFF, sizeof(ValueT) * num_queries));
    ...
    CUDA_CALL(cudaMemcpy(h_result, d_result_, sizeof(ValueT) * num_queries,
                         cudaMemcpyDeviceToHost));
    cudaDeviceSynchronize();
    return temp_time;
  }

  float hash_search_bulk(KeyT* h_query, ValueT* h_result,
  ...
  float hash_count(KeyT* h_query, uint32_t* h_count, uint32_t num_queries) {
    ...
    CUDA_CALL(cudaMemcpy(d_query_, h_query, sizeof(KeyT) * num_queries,
                         cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMemset(d_result_, 0x00, sizeof(uint32_t) * num_queries));
    ...
    CUDA_CALL(cudaMemcpy(h_count, d_count_, sizeof(uint32_t) * num_queries,
                         cudaMemcpyDeviceToHost));
    cudaDeviceSynchronize();
    return temp_time;
  }
  ...
  float batched_operations(uint32_t* h_batch_op, uint32_t* h_results,
                           uint32_t batch_size, uint32_t batch_id) {
    CUDA_CALL(cudaSetDevice(device_idx_));
    CUDA_CALL(cudaMemcpy(d_key_ + batch_id * batch_size, h_batch_op,
                         sizeof(uint32_t) * batch_size,
                         cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMemset(d_result_ + batch_id * batch_size, 0xFF,
                         sizeof(uint32_t) * batch_size));

...
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap>::countIndividual(
    KeyT* d_query, uint32_t* d_count, uint32_t num_queries) {
  CUDA_CALL(cudaSetDevice(device_idx_));
  const uint32_t num_blocks = (num_queries + BLOCKSIZE_ - 1) / BLOCKSIZE_;
...
GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap>::computeLoadFactor(
  ...
  CUDA_CALL(cudaMalloc((void**)&d_bucket_pairs_count,
                       sizeof(uint32_t) * num_buckets_));
  CUDA_CALL(
      cudaMemset(d_bucket_pairs_count, 0, sizeof(uint32_t) * num_buckets_));

  uint32_t* h_bucket_slabs_count = new uint32_t[num_buckets_];
  uint32_t* d_bucket_slabs_count;
  CUDA_CALL(cudaMalloc((void**)&d_bucket_slabs_count,
                       sizeof(uint32_t) * num_buckets_));
  CUDA_CALL(
      cudaMemset(d_bucket_slabs_count, 0, sizeof(uint32_t) * num_buckets_));
  ...
  CUDA_CALL(cudaMemcpy(h_bucket_pairs_count, d_bucket_pairs_count,
                       sizeof(uint32_t) * num_buckets_,
                       cudaMemcpyDeviceToHost));
  CUDA_CALL(cudaMemcpy(h_bucket_slabs_count, d_bucket_slabs_count,
                       sizeof(uint32_t) * num_buckets_,
                       cudaMemcpyDeviceToHost));
  ...
  if (d_bucket_pairs_count) CUDA_CALL(cudaFree(d_bucket_pairs_count));
  if (d_bucket_slabs_count) CUDA_CALL(cudaFree(d_bucket_slabs_count));

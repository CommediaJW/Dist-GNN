...
#pragma hd_warning_disable
  ...
  __host__ void initParameters(const uint32_t num_buckets,
                               const uint32_t hash_x, const uint32_t hash_y,
                               int8_t* d_table,
  ...
  __device__ __forceinline__ void insertPair(
      bool& to_be_inserted, const uint32_t& laneId, const KeyT& myKey,
      const ValueT& myValue, const uint32_t bucket_id,
      AllocatorContextT& local_allocator_context);

  // threads in a warp cooperate with each other to insert a unique key (and its
  // value) into the slab hash
  ...
  __device__ __forceinline__ void searchKey(bool& to_be_searched,
                                            ...
                                            const KeyT& myKey, ValueT& myValue,
  ...
  __device__ __forceinline__ void countKey(bool& to_be_searched,
                                           const uint32_t& laneId,
                                           const KeyT& myKey, uint32_t& myCount,
                                           const uint32_t bucket_id);
...
class GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap> {
 ...
 public:
  GpuSlabHash(const uint32_t num_buckets, DynamicAllocatorT* dynamic_allocator,
              uint32_t device_idx, const time_t seed = 0,
    ...
    CUDA_CALL(cudaMalloc((void**)&d_table_, slab_unit_size_ * num_buckets_));

    CUDA_CALL(cudaMemset(d_table_, 0xFF, slab_unit_size_ * num_buckets_));

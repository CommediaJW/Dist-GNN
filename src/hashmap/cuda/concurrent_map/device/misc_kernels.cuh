...
/*
 * This kernel can be used to compute the total number of elements and the total
 * number of slabs per bucket. The final results per bucket is stored in
 * d_pairs_count_result and d_slabs_count_result arrays respectively
 */
template <typename KeyT, typename ValueT>
__global__ void bucket_count_kernel(
    GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap> slab_hash,
    uint32_t* d_pairs_count_result, uint32_t* d_slabs_count_result,
  ...
  while (next != SlabHashT::EMPTY_INDEX_POINTER) {
    ...
    pairs_count +=
        __popc(__ballot_sync(0xFFFFFFFF, src_unit_data != EMPTY_KEY) &
               SlabHashT::REGULAR_NODE_KEY_MASK);

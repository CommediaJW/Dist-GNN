      ...
      DGS_VALUE_TYPE_SWITCH(cpu_probs.dtype(), FloatType, {
        ...
        thrust::for_each(
            ...
            [seeds = seeds.data_ptr<NType>(),
             ...
             row_pos = pos_list.data_ptr<NType>(), gpu_hashmap_size,
             num_items] __device__(int64_t i) mutable {
              ...
              EType begin, end;
              if (pos != -1) {
                begin = gpu_indptr->At(gpu_hashmap_devid[pos],
                                       gpu_hashmap_idx[pos]);
              ...
              EType deg = end - begin;
              EType sub_offset, temp_offset;
              if (replace) {
                sub_offset = deg == 0 ? 0 : num_picks;
                temp_offset = deg;
              } else {
                sub_offset = MIN(deg, num_picks);
                temp_offset = deg > num_picks ? deg : 0;
              }
              sub_indptr[i] = sub_offset;
              temp_indptr[i] = temp_offset;
